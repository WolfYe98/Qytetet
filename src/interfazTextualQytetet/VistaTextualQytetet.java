...
public int menuGestionInmobiliaria(){ //ejemplo de menú
    ...
    int salida=this.seleccionMenu(menuGI); // Método para controlar la elecci�n correcta en el menú 
    return salida;
 }

public int menuSalirCarcel(){
    this.mostrar("\nElige el metodo de salir carcel que quiera usar");
    Map<Integer, String> menuGI = new TreeMap();
    menuGI.put(0, "Tirando el dado"); 
    menuGI.put(1, "Usando la carta de libertad");
    int salida=this.seleccionMenu(menuGI);  
    return salida;
 }
  
 public boolean elegirQuieroComprar(){
     this.mostrar("Calle sin propietario ¿Comprar?");
     Map<Integer, String> menuGI = new TreeMap();
    menuGI.put(0, "Si"); 
    menuGI.put(1, "no");
    int salida=this.seleccionMenu(menuGI);
...
//m�todo para comprobar que se introduce un entero correcto, usado por seleccion_menu
 ...
 public void pausa(){
     System.out.println("Pulse algo para continuar");
     in.nextLine();
 }
